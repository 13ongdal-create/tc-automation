...
- **[추가] `/tc-test` 슬래시 커맨드**: 이미 생성된 자동화 테스트를 재실행(회귀 테스트 등)하는 독립 진입점입니다.
  1. Slack API 앱 페이지 → **Slash Commands** → **Create New Command**
  2. Command: `/tc-test`, Request URL: (Socket Mode라 미사용, 임시로 `https://example.com/tc-test`), Usage Hint: `프로젝트=ABC마트 모듈=장바구니`
...
```
/tc-generate 프로젝트=ABC마트 모듈=장바구니
```

URL을 추가하면 정책서 대신(또는 함께) 실제 화면을 Playwright로 관찰해 근거로 삼습니다:

```
/tc-generate 프로젝트=ABC마트 모듈=장바구니 URL=https://abc-mart.example.com/cart
```

목표 건수는 미리 정하지 않습니다 — 근거에서 도출 가능한 시나리오를 최대한 뽑아서 실제 개수를 그대로 보고합니다.

1. (신규 프로젝트라면) 봇이 먼저 URL/단위·통합 구분/코드·정책기반 여부를 되물어봅니다 (Phase 0).
2. Phase 1(분석) 결과를 올리고 승인을 기다립니다. **각 Phase(1~4)는 개별적으로 승인**해야 다음으로 진행됩니다 — 한 번에 묶어 진행하지 않습니다.
3. 같은 스레드에 답장으로 수정 요청 가능: `우선순위 재조정해줘`, `쿠폰 관련 케이스 더 추가해줘` 등 자유 텍스트.
4. 승인: 스레드에 `승인`이라고 답장 → 바로 다음 Phase 하나만 진행합니다.
5. Phase 4(TC 작성)까지 승인돼도 **자동화 테스트는 실행되지 않습니다.** 아래 "테스트 실행" 섹션 참고.
6. 반려: `반려: 사유` 형식으로 답장 → 사유를 반영해 재작성합니다.
7. 중단: 진행 중(생성/자동화 테스트 실행 등)에 `중단` / `취소` / `그만` / `stop` / `cancel` 이라고 답장 → 실행 중인 프로세스를 트리 전체(하위 git/playwright 포함) 강제 종료하고 중단 안내 메시지를 남깁니다. 이후 같은 스레드에 새 요청을 이어서 입력할 수 있습니다.
8. 진행 상태 확인: 처리 중에 `상태` / `진행상황` / `progress` 라고 답장하면 **그 자리에서 즉시** 지금 어떤 작업(예: `Read: Policy/...docx`, `Bash: npx playwright test ...`) 중인지 알려줍니다. Claude를 다시 호출하지 않고 이미 실행 중인 프로세스의 스트리밍 출력에서 뽑아오는 것이라 **토큰을 추가로 쓰지 않습니다**. (v0.2부터: 예전처럼 몇 분마다 자동으로 "처리 중" 알림을 올리던 방식은 스레드가 알림으로 도배되는 문제가 있어 제거하고, 필요할 때 물어보는 방식으로 바꿨습니다.)

**테스트 실행(Phase 5)**은 Phase 4(TC 작성)와 별개로 명시적으로 요청해야 합니다. 두 가지 방법이 있습니다:

- TC 생성 스레드 안에서: `테스트 실행해줘` / `테스트 수행해줘` 라고 답장
- 독립적으로(회귀 테스트 등, 새 스레드): `/tc-test 프로젝트=ABC마트 모듈=장바구니` — 이미 생성된 자동화 테스트 코드를 그대로 재실행

실행되면 콘솔 에러/실패 감지 → 스크린샷 첨부 → `defects.json` 결함 등록/갱신 → Pass/Fail 요약까지 이어지고, 이 결과를 최종 승인해야 git commit/push가 이루어집니다.
