...
<!-- [추가 2026-08-28] 팀원 여러 명이 각자 로컬에서 동시에 작업하는 협업 시나리오가 실제로
검토/문서화(대시보드 팀 공유 셋업가이드, AGENTS.md 18항 브랜치 전략)된 뒤에도 이 4번 절이
"①/② 둘 중 하나"로만 안내하고 있어 그 사이의 세 번째 시나리오가 빠져있던 것을 보완. -->
- **③ 팀원 여러 명이 각자 로컬에서 동시에 작업하는 경우(①/②와 다름 — 협업)**: 혼자 이관받는
  게 아니라 여러 사람이 동시에 이 저장소를 쓰는 상황이면, `_reference\대시보드_팀_공유_셋업가이드.md`
  (각자 로컬 대시보드 실행 + git 브랜치 전략)와 `AGENTS.md` 18항의 조건부 브랜치 규칙(프로젝트별
  `{프로젝트명}/{작업요약}`, 공통 파일은 `common/{작업요약}` 브랜치)을 먼저 읽고 시작하세요 —
  혼자 세션을 여는 평소 방식(메인 브랜치에 직접 커밋)과는 다른 절차입니다.
...
- 규칙/스킬 변경 이력을 요약하는 별도 페이지: **📘 Claude QA 자동화 (규칙/스킬/워크플로우)** — 최신
  ...
  섹션 누적 방지, 최신 1건만 유지).
- 자세한 규칙은 `.claude/skills/project-onboarding/SKILL.md`의 노션 동기화 절차(7번 항목) 참조.
...
| Slack 스레드에서 "DEF_001 담당자를 홍길동으로 지정해줘" | 같은 문구 그대로 요청 (동작 동일, `defect-management` 스킬) |
| Slack이 실패 스크린샷을 스레드에 자동 첨부 | 실패 보고 시 Claude가 `Read` 도구로 스크린샷을 직접 열어 응답에 보여줌 (`playwright-automation` 스킬) |
...
<!-- [추가 2026-08-28] 이 저장소를 "이어받는" 것(4번)과 이 방법론을 "완전히 다른 프로젝트/저장소로
내보내는" 것은 서로 다른 시나리오인데, 후자를 위한 문서(PORTABLE_QA_METHODOLOGY.md)가 새로
추가된 뒤에도 온보딩 문서에서 전혀 연결되어 있지 않아 존재 자체를 못 찾을 수 있어 보완. -->
## 9. 이 방법론을 완전히 다른 프로젝트/저장소로 내보내기

지금까지의 4~8번은 모두 **"이 `tc-automation` 저장소 자체를 이어받는"** 상황을 다룹니다. 만약 목적이
그게 아니라 **이커머스가 아닌 다른 도메인/완전히 별개의 저장소에 이 TC·결함관리 방법론과 Phase 0~8
승인 파이프라인만 동일하게 재현**하는 것이라면, 저장소 루트의 **`PORTABLE_QA_METHODOLOGY.md`** 파일
하나만 새 저장소에 복사하면 됩니다.

- 이 문서는 Claude/Notion/Slack 등 **이 저장소의 구체적 구현체에 대한 종속성을 걷어낸** 버전입니다 —
  TC 컬럼 스키마·HTML 뷰어 스펙·결함 스키마·Phase 0~8 워크플로우는 원본과 동일한 상세도로 보존하되,
  AI 도구(Claude가 아닌 Gemini/ChatGPT 등)나 협업 툴(Notion이 아닌 Jira/Redmine 등)은 자유롭게
  바꿔 낄 수 있도록 어댑터 노트(24장)로 분리되어 있습니다.
- `agents-config\AGENTS.md`/`SKILL.md`를 그대로 복사해가지 않는 이유: 그 두 파일은 이 저장소의 실제
  운영 규칙이라 Claude Code 전용 표현·Notion MCP 구체 절차·이 저장소만의 파일 경로가 곳곳에 섞여
  있습니다. 다른 프로젝트에 그대로 옮기면 도구/경로 불일치로 오히려 혼란을 줍니다 —
  `PORTABLE_QA_METHODOLOGY.md`가 그 일반화 작업을 이미 끝내둔 버전입니다.
